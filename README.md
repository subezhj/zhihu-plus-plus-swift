...
> - **120Hz + UI 优化（Tag [`v1.4.0-120hz-ui-optimization`](https://github.com/subezhj/zhihu-plus-plus-swift/tree/v1.4.0-120hz-ui-optimization)）**：设置项「强制 120Hz 高刷 (ProMotion)」；回答页纯 SwiftUI 线性分页、评论/写评论 UI 液态玻璃化；评论区加载对齐首页（占位/下拉刷新/取消重置/同步测量）；评论内存缓存 + 120s 过期静默刷新；Haptic 复用与柔和曲线。
> - **新架构重构（Tag v1.4.0 之后）**：抽象 `PagingSource` 协议 + `PagingListContent` 通用分页组件（首页推荐流已接入），逐步收敛各列表页的分页/刷新/占位实现。
